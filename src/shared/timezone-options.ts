...
/** 写入前校验：只有白名单内的 value 才放行（native 设置窗等外部写入路径用）。 */
export function isAllowedTimezoneValue(raw: unknown): raw is string {
  return typeof raw === "string" && TIMEZONE_OPTIONS.some((o) => o.value === raw);
}