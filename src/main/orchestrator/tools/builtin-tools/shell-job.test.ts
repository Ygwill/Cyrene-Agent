...
describe("shell_job 工具协议（注册 + run_shell 后台集成）", () => {
  ...
  // pid 断言是 Windows 语义（Linux detached spawn 可得 pid=0）
  it.skipIf(process.platform !== "win32")("run_shell run_in_background 集成：启动返回 jobId/logFile → status running → stop → 进程真死", async () => {
